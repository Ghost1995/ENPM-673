...
function I = detectBuoy(gmObjs, frame)
    ...
    % Read the image
    I = imread(frame);
    imgData = double(reshape(I,numel(I)/3,3));

    % Compute gaussian probabilities
    greenProb = reshape(gauss(greenGauss,imgData(:,1)),size(I,1),size(I,2));
    redProb = reshape(gauss(redGauss,imgData(:,1)),size(I,1),size(I,2));
    yellowProb = reshape(gauss(yellowGauss,imgData(:,1)),size(I,1),size(I,2));
    
    % Identify green buoy
    greenBuoy = greenProb <1.5*std2(greenProb);
    greenBuoy = bwareafilt(imfill(greenBuoy,'holes'),[150 425]);
    ...
    redBuoy = bwareafilt(imfill(redBuoy,'holes'),[275 6000]);
    ...
    % Save the image
    imwrite(greenBuoy | redBuoy | yellowBuoy,['.\ColorSeg\Output\Part3\RGB\binary_' frame(end-6:end)]);
    imwrite(I,['.\ColorSeg\Output\Part3\RGB\Frames\out_' frame(end-6:end)]);

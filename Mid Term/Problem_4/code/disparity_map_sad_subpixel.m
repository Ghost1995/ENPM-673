%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This code generates disparity map using SAD for defined window size and
% then computes subpixel disparity.
% 
% Submitted by: Ashwin Goyal (UID - 115526297)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function disparity_map_sad_subpixel(I_left, I_right, window_size)
...
for i=ceil(window_size/2):floor(window_size/2)+size(I_left,1)
    for j=ceil(window_size/2):floor(window_size/2)+size(I_left,2)
        ...
        for k=ceil(window_size/2):floor(window_size/2)+size(I_left,2)
            ...
            % Compute SAD
            SAD(k-floor(window_size/2)) = sum(sum(abs(left_window - right_window)));
        end
        % Compute disparity at subpixel level
        min_pos = find(SAD==min(SAD),1);
        if (min_pos~=1)&&(min_pos~=length(SAD))&&(SAD(min_pos-1) - 2*SAD(min_pos) + SAD(min_pos+1) ~= 0)
            D(i-floor(window_size/2),j-floor(window_size/2)) = abs(min_pos - j - (0.5*(SAD(min_pos+1) - SAD(min_pos-1)))/(SAD(min_pos-1) - 2*SAD(min_pos) + SAD(min_pos+1)));
        else
...
% Save the image
saveas(gca,['../output/SAD_subpixel/Disparity Map using SAD_subpixel for window_size = ' num2str(window_size) '.jpg'])
